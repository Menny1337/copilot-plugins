...
// daemon (= once per tab) — so reusing a daemon keeps that single approval
// alive across many commands. Daemons auto-exit after the idle timeout
// (default 60min; override with env CDP_IDLE_TIMEOUT, in minutes).
...
// Daemon idle timeout. A longer-lived daemon keeps the user's single "Allow
// debugging" approval alive across a whole task (fewer re-prompts), at the cost
// of holding a CDP session open longer. Override with env CDP_IDLE_TIMEOUT
// (minutes); tear down explicitly with `cdp.mjs stop [target]`.
const IDLE_TIMEOUT = (() => {
  const raw = process.env.CDP_IDLE_TIMEOUT;
  const mins = raw != null && raw !== '' ? Number(raw) : 60;
  if (!Number.isFinite(mins) || mins <= 0) return 60 * 60 * 1000;
  return mins * 60 * 1000;
})();
const DAEMON_CONNECT_RETRIES = 20;
const DAEMON_CONNECT_DELAY = 300;
const MIN_TARGET_PREFIX_LEN = 8;
const CDP_BROWSER = (process.env.CDP_BROWSER || 'auto').toLowerCase();
const CDP_PORT_OVERRIDE = process.env.CDP_DEVTOOLS_ACTIVE_PORT;
const BROWSER_SCOPE = CDP_PORT_OVERRIDE
  ? `custom-${Buffer.from(resolve(CDP_PORT_OVERRIDE)).toString('base64url').slice(0, 12)}`
  : CDP_BROWSER;
const TMP_DIR = tmpdir();
const DAEMON_PREFIX = `cdp-${BROWSER_SCOPE}-`;
const PAGES_CACHE = join(TMP_DIR, `cdp-pages-${BROWSER_SCOPE}.json`);
const IS_WIN = platform() === 'win32';

// DevToolsActivePort paths per browser per platform
const BROWSER_PORT_FILES = (() => {
  const home = homedir();
  if (IS_WIN) {
...
// Daemon IPC: TCP on all platforms (cross-platform)
function portFilePath(targetId) { return join(TMP_DIR, `${DAEMON_PREFIX}${targetId}.port`); }

function getWsUrl() {
  let candidates;
  if (CDP_PORT_OVERRIDE) {
    candidates = [resolve(CDP_PORT_OVERRIDE)];
  } else if (CDP_BROWSER === 'auto') {
    candidates = [...BROWSER_PORT_FILES.chrome, ...BROWSER_PORT_FILES.edge];
  } else if (BROWSER_PORT_FILES[CDP_BROWSER]) {
    candidates = BROWSER_PORT_FILES[CDP_BROWSER];
  } else {
    throw new Error(`Unsupported CDP_BROWSER "${CDP_BROWSER}". Use "auto", "chrome", or "edge".`);
  }
...
function listDaemons() {
  try {
    return readdirSync(TMP_DIR)
      .filter(f => f.startsWith(DAEMON_PREFIX) && f.endsWith('.port'))
      .map(f => {
        const targetId = f.slice(DAEMON_PREFIX.length, -5);
        const portFile = join(TMP_DIR, f);
        let port;
        try { port = parseInt(readFileSync(portFile, 'utf8').trim(), 10); } catch { return null; }
        if (!port || isNaN(port)) return null;
        return { targetId, port, portFile };
      })
      .filter(Boolean);
  } catch { return []; }
}
...
// Keyboard: dispatch key events (Enter, Tab, Escape, etc.)
const KEY_MAP = {
...
async function keyStr(cdp, sid, keyName) {
  if (!keyName) throw new Error('Key name required (e.g. Enter, Tab, Escape)');
  const mapped = KEY_MAP[keyName.toLowerCase()];
  const keyDef = mapped || { key: keyName, code: keyName, keyCode: 0 };
  const base = {
  ...
  while (Date.now() < deadline) {
    ...
    if (exists === 'true') {
      const info = await evalStr(cdp, sid,
        `(function() { const el = document.querySelector(${JSON.stringify(selector)}); return el ? el.tagName + ': ' + el.textContent.trim().substring(0, 60) : ''; })()`
      );
      return `Found ${selector} → ${info}`;
    }
    await sleep(250);
  }
  throw new Error(`Timeout (${timeout}ms): selector "${selector}" not found`);
}

// Full-page screenshot: capture entire scrollable page using CSS metrics
...
async function runDaemon(targetId) {
  const pf = portFilePath(targetId);
  ...
  async function handleCommand({ cmd, args }) {
    ...
    try {
      ...
      switch (cmd) {
        ...
        case 'key': case 'press': result = await keyStr(cdp, sessionId, args[0]); break;
  ...
  // TCP server — NDJSON protocol (cross-platform, replaces Unix socket)
  ...
  // Listen on localhost with dynamic port, write port file for clients
  server.listen(0, '127.0.0.1', () => {
    const { port } = server.address();
    writeFileSync(pf, String(port));
  });
}
...
async function getOrStartTabDaemon(targetId) {
  // Try existing daemon via port file
  const pf = portFilePath(targetId);
  if (existsSync(pf)) {
    try {
      const port = parseInt(readFileSync(pf, 'utf8').trim(), 10);
      if (port) return await connectToDaemon(port);
    } catch {}
    // Stale port file
    try { unlinkSync(pf); } catch {}
  }
  ...
  throw new Error('Daemon failed to start — did you click Allow debugging in the browser?');
}
...
function findAnyDaemonPort() {
  const d = listDaemons()[0];
...
async function stopDaemons(targetPrefix) {
  ...
  if (targetPrefix) {
    ...
    try {
      const conn = await connectToDaemon(daemon.port);
      await sendCommand(conn, { cmd: 'stop' });
    } catch {
      try { unlinkSync(daemon.portFile); } catch {}
    }
    return;
  }
...
const USAGE = `cdp - raw Chrome/Edge DevTools Protocol escape hatch (no Puppeteer)

This is the LOW-LEVEL escape hatch for the browser skill. For routine
browser interaction (open, snapshot, click, type, fill, screenshot,
multi-tab, persistent profiles, M365/Entra SSO) prefer playwright-cli:
  npm install -g @playwright/cli@latest

Use cdp.mjs only when:
  - You need a raw CDP method (Network.*, Performance.*, Target.*, etc.)
  - You need to attach to the user's already-running Chrome/Edge without
    installing a Playwright extension
  - You need to type into a cross-origin iframe (Input.insertText path)
  - playwright-cli's run-code can't express what you need
...
Usage: cdp <command> [args]
  ...
  key     <target> <key>            Press a key (Enter, Tab, Escape, Backspace, ArrowDown, Space, etc.)
  wait    <target> <selector> [ms]  Wait for a CSS selector to appear (default 10000ms timeout)
  evalraw <target> <method> [json]  Send a raw CDP command; returns JSON result
                                    e.g. evalraw <t> "DOM.getDocument" '{}'
                                    e.g. evalraw <t> "Network.setBlockedURLs" '{"urls":["*.jpg"]}'
  stop    [target]                  Stop daemon(s)

Removed in 2026-05 (use playwright-cli instead):
  shot/screenshot, fullshot         → playwright-cli screenshot [--filename=<f>]
  html                              → playwright-cli snapshot   (or eval "el => el.outerHTML")
  click                             → playwright-cli click <ref|selector>
  hover                             → playwright-cli hover <ref|selector>
  scroll                            → playwright-cli mousewheel <dx> <dy>

Environment:
  CDP_BROWSER=edge                  Use Microsoft Edge instead of Chrome/auto-detect
  CDP_DEVTOOLS_ACTIVE_PORT=<path>   Use a specific DevToolsActivePort file

<target> is a unique targetId prefix from "cdp list". If a prefix is ambiguous,
use more characters.

COORDINATE SYSTEM (clickxy)
  CDP Input events take CSS pixels. If you have screenshot pixels:
    CSS pixels = screenshot image pixels / DPR
  Typical Retina (DPR=2): CSS px ≈ screenshot px × 0.5
  Use playwright-cli for screenshots — it handles DPR transparently.
...
DAEMON IPC (for advanced use / scripting)
  Each tab runs a persistent daemon on TCP localhost with a dynamic port.
  Port is stored in: <tmpdir>/cdp-<scope>-<fullTargetId>.port
  Protocol: newline-delimited JSON (one JSON object per line, UTF-8).
    Request:  {"id":<number>, "cmd":"<command>", "args":["arg1","arg2",...]}
    Response: {"id":<number>, "ok":true,  "result":"<string>"}
           or {"id":<number>, "ok":false, "error":"<message>"}
  Commands mirror the CLI: snap, eval, nav, net, clickxy, type, loadall,
  evalraw, key, wait, stop. Use evalraw to send arbitrary CDP methods.
  The port file disappears after 20 min of inactivity or when the tab closes.
`;

const NEEDS_TARGET = new Set([
  'snap','snapshot','eval','nav','navigate',
  'net','network','clickxy','type','loadall','evalraw',
  'key','press','wait',
]);
...
async function main() {
  ...
  // Runtime guard: this script uses the global WebSocket (Node 22+). On older
  // Node it would otherwise fail with a cryptic ReferenceError deep in connect().
  if (typeof WebSocket === 'undefined') {
    process.stderr.write(
      `cdp.mjs requires Node 22+ (built-in WebSocket); current runtime is ${process.version}.\n` +
      'Re-run under Node 22 or newer (e.g. `nvm use 22`, or invoke an explicit node@22 binary).\n'
    );
    process.exit(1);
  }
  ...
  if (cmd === 'list' || cmd === 'ls') {
    let pages;
    const existingPort = findAnyDaemonPort();
    if (existingPort) {
      try {
        const conn = await connectToDaemon(existingPort);
  ...
  let targetId;
  const daemonTargetIds = listDaemons().map(d => d.targetId);
