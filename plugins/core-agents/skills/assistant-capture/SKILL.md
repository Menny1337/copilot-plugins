...
Every write in §1, §2, §3, §4, and §5 (local-store operations only — not §3.5/§8 ADO)
...
If `backend != "ado"`, fall back to §3 markdown behavior. Otherwise, extract:
...
To stay on the local-markdown backend instead, set `"taskBackend": "markdown"` (or omit the file entirely); the ADO keys are then ignored. Other features layer their own keys onto this same file (e.g. `adoSessionSync.enabled` — see the `ado-session-sync` skill).
